...
public class PacienteController {
    ...
    public ResponseEntity<Page<DadosListagemPaciente>>listar(@PageableDefault(size = 10, sort = "nome") Pageable page){
        var pagina = repository.findByAtivoTrue(page).map(DadosListagemPaciente:: new);
    ...
    @PutMapping
    @Transactional
    public  ResponseEntity atualziar(@RequestBody @Valid DadosAtualizacaoPaciente dados){
